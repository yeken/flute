...
class Ion_auth_model extends CI_Model
{
	...
	public function change_password($identity, $old, $new)
	{
	    ...
	    $query = $this->db->select('user_id, password, salt')
	    ...
	    $old	     = $this->hash_password_db($result->user_id, $old);
	...
	public function register($username, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		if (isset($default_group->group_id) && isset($groups) && !empty($groups) && !in_array($default_group->group_id, $groups) || !isset($groups) || empty($groups))
		{
	...
	public function login($identity, $password, $remember=FALSE)
	{
	    ...
	    $query = $this->db->select('username, email, user_id, password, last_login')
		...
		if ($query->num_rows() == 1)
	    {
			$password = $this->hash_password_db($user->user_id, $password);
	...
	public function get_users_groups($id=FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as group_id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description')
						->where($this->tables['users_groups'].'.'.$this->join['users'], $id)
						->join($this->tables['groups'], $this->tables['users_groups'].'.'.$this->join['groups'].'='.$this->tables['groups'].'.group_id')
	...
	public function login_remembered_user()
	{
	    ...
	    //get the user
	    $this->trigger_events('extra_where');
	    $query = $this->db->select($this->identity_column.', user_id')
	    ...
	    if ($query->num_rows() == 1)
	    {
			...
			$session_data = array(
			                    ...
			                    'id'                   => $user->user_id, //kept for backwards compatibility
			                    'user_id'              => $user->user_id, //everyone likes to overwrite id so we'll use user_id
			                     );

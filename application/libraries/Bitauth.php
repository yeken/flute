...
class Bitauth
{
	 ...
	 */
	public function login($username, $password, $remember = FALSE, $extra = array(), $token = NULL, $company_id = "")
	{
		if(empty($username))
		{
		...
		$user = $this->get_user_by_username($username,NULL, $company_id);
	...
	public function check_existing_username($username, $id = NULL)
	{
		...
		$sql = "SELECT * FROM ".$this->_table['users']." WHERE username = '".$username."' ".($id ? " AND user_id != '".$id."'" : "");
	...
	public function add_user($data, $require_activation = NULL)
	{
		...
		if($this->check_existing_username($data['username']))
		{
			$this->set_error("El nombre de usuario ".$data['username']." ya existe");
			return FALSE;
			
		}
		
		if($this->check_existing_email($data['email']))
		{
			$this->set_error("Ya hay una cuenta registrada con el email ".$data['email']);
			return FALSE;
		...
		if($require_activation)
		{
			$this->send_activation_email($data['activation_code'],$user_id);
		}
	...
	public function send_new_password_email($user, $password)
	{
		$data['password'] = $password;
		$data['username'] = $user->username;
		
		$view = $this->load->view("templates/user_new_password",$data, true);
		...
		$this->email_lib->from($this->configuration_model->email,$this->configuration_model->email_name);
		$this->email_lib->to($user->email);
		
		$this->email_lib->subject('Nueva contrase&ntilde;a de '.$this->configuration_model->site_name);
		$this->email_lib->message($view);

		return $this->email_lib->send();			
	}
	
	public function send_activation_email($activation_code, $user_id)
	{
		$data['activation_link'] = base_url()."usuario/activate/".$activation_code;
		
		$user = $this->get_user_by_id($user_id);
		
		$data['fullname'] = $user->fullname;
		
		$view = $this->load->view("templates/user_account_activation",$data, true);
		...
		$this->email_lib->subject('Activ&aacute; tu cuenta');
		$this->email_lib->message($view);

		return $this->email_lib->send();
	}

	public function send_forgot_password_email($forgot_link, $user_id)
	{
		...
		$view = $this->load->view("templates/user_forgot_password",$data, true);
		...
		$this->email_lib->subject('Recuper&aacute; tu clave');
		$this->email_lib->message($view);
		return $this->email_lib->send();
	}
	...
	public function activate($activation_code)
	{
		...
		{
			$this->update_user($user->user_id, array('active' => 1, 'activation_code' => ''));
			return $user;
		}
	...
	public function activate_by_id($user_id)
	{
	...
	public function update_user($id, $data)
	{
		...
		if(isset($data['username']) && $this->check_existing_username($data['username'],$id))
		{
	...
	public function get_users_by_group($group_id)
	{
		...
		if($query && $query->num_rows())
		{
			...
			foreach($result as $row)
			{
				...
				$row->user_image_url = base_url().(($row->image_file_name!='')?'uploads/files/'.$row->image_file_name:'assets_fe/images/defaults/user_picture.jpg');
				$ret[] = $row;
			}
	...
	public function get_users($include_disabled = FALSE)
	{
		...
		if($query && $query->num_rows())
		{
			...
			foreach($result as $row)
			{
				...
				$row->roles = $roles;
				$row->last_login_ip = long2ip($row->last_login_ip);
				$row->user_image_url = base_url().'uploads/files/'.$row->image_file_name;
				$ret[] = $row;
			}
	 ...
	 */
	public function get_user_by_username($username, $include_disabled = FALSE, $company_id = "")
	{
	...
	public function get_user_by_email($email, $include_disabled = FALSE, $company_id = "")
	{
	...
	{
		$sql = "SELECT * FROM bitauth_users WHERE user_id = '".$this->user_id."' AND group_id = '".(int)$group_id."'";
	...
	{
		$query = $this->db
			->select('groups.*, GROUP_CONCAT(users.user_id SEPARATOR "|") AS members')
			->join('bitauth_users', 'bitauth_users.group_id = groups.group_id', 'left')
